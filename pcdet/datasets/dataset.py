...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.tag = self.dataset_cfg.tag
        self.save_path = self.root_path / 'infos' / self.tag
        self.logger = logger
        ...
        self.data_augmentor = DataAugmentor(
            self.root_path, self.save_path, self.dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger
    ...
    def get_img_feats(self, idx):

        feats_file = self.feats_dir / ('%s.npz' % idx)
        assert feats_file.exists()
        feats = np.load(feats_file)['arr_0']
